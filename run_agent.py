...
class AIAgent:
    ...
    def __init__(
        ...
        self.session_start = datetime.now(ZoneInfo("Asia/Bangkok"))
        if session_id:
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            timestamp = datetime.now(ZoneInfo("Asia/Bangkok")).strftime("%Y%m%d_%H%M%S_%f")
    ...
    def run_conversation(
        ...
        try:
            """Inject context-mode retrieval context (ephemeral, non-authoritative).

            - Task B: Passive retrieval (skill-based context)
            - Task D: Active retrieval (proactive pattern-based context) <- NEW
            """
            # Task B hook (existing - Context Mode passive retrieval)
            ...
            # Task D hook (NEW - Active Context Retrieval with pattern detection)
            try:
                from agent.active_context_retrieval import build_retrieval_block_for_agent
                _active_context = build_retrieval_block_for_agent(
                ...
                logger.debug("Active Context Retrieval hook failed: %s", exc)
        except Exception as exc:
