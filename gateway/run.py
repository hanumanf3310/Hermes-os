...
class GatewayRunner:
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        """Handle /reload-skills — rescan skills and queue a next-turn note."""
        ...
        try:
            ...
            # Allow adapters with cached skill UI state (e.g. Discord
            # autocomplete) to refresh in-place. Telegram/Gateway command
            # dispatch relies on the registry and skill map refreshed above.
    ...
    async def _run_agent(
        ...
        def run_sync():
            ...
            _pending_skill_notes = getattr(self, '_pending_skills_reload_notes', {})
            _srn = _pending_skill_notes.pop(session_key, None) if session_key else None
            if _srn:
                message = _srn + "\n\n" + message
