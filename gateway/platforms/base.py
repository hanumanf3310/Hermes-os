...
def resolve_proxy_url(platform_env_var: str | None = None, target_hosts: list[str] | tuple[str, ...] | None = None) -> str | None:
    """Return a proxy URL from env vars, or macOS system proxy.

    Args:
        platform_env_var: Optional platform-specific proxy env var
            (for example ``TELEGRAM_PROXY``).
        target_hosts: Optional host list accepted for compatibility with
            platform adapters that probe/direct-route specific hosts. The
            current resolver does not need the value and keeps the legacy
...
def resolve_channel_prompt(extra: dict | None, channel_id: str | None, parent_id: str | None = None) -> str | None:
    """Resolve a per-channel prompt from platform adapter config.

    The exact channel/topic id wins over the parent chat/channel id. Blank prompts
    are ignored. Config migration is responsible for normalizing YAML keys to
    strings, so this resolver intentionally only matches string keys.
    """
...
class MessageEvent:
    ...
    # Per-channel/topic ephemeral prompt injected for this event only.
