...
/// OpenAI/OpenAI Responses can also use `OPENAI_BASE_URL` for a custom
/// OpenAI-compatible endpoint, and Ollama uses a local endpoint with no auth
/// (base URL overridable via `OLLAMA_BASE_URL`).
pub fn build_provider(config: &AppConfig) -> Result<Arc<dyn Provider>> {
    ...
    let api_key = match config.api_key_from_env() {
        Some(key) => key,
        None if matches!(kind, ProviderKind::OpenAI | ProviderKind::OpenAIResponses)
    ...
    match kind {
        ...
        ProviderKind::OpenAI => {
            let api_key = if api_key.is_empty() && openai_base_url.is_some() {
                String::new()
            } else {
                api_key
            };
            let mut provider = OpenAIProvider::new(api_key);
            if let Some(url) = openai_base_url.clone() {
        ...
        ProviderKind::OpenAIResponses => {
            ...
            let mut provider = crate::providers::openai_responses::OpenAIResponsesProvider::new(api_key);
...
mod tests {
    ...
    fn build_provider_honors_env_keys() {
        ...
        // Case 3: openai model + custom base URL + no key → builds as a
        // keyless OpenAI-compatible provider.
        std::env::set_var("OPENAI_BASE_URL", "http://127.0.0.1:18081/v1/chat/completions");
        let mut openai_cfg = AppConfig::default();
        openai_cfg.model = "gpt-4o".into();
        build_provider(&openai_cfg).expect("openai should build with custom base url");
        std::env::remove_var("OPENAI_BASE_URL");

        // Case 4: openai model + openai key → builds openai.
