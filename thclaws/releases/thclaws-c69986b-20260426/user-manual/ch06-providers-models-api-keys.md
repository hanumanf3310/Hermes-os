...
> `thclaws-safe` wrapper behavior: when `OPENAI_API_KEY` is missing, it first tries a reachable local OpenAI-compatible endpoint; if none is reachable, it falls back to local Ollama (`ollama/qwen3.5:cloud`).
...
| OpenAI | `gpt-*`, `o1-*`, `o3*`, `o4-*` | `OPENAI_API_KEY` (or `OPENAI_BASE_URL` for a custom OpenAI-compatible endpoint) | Chat Completions; automatic prompt caching; keyless endpoints work when the custom endpoint does not require auth |
| Ollama | `ollama/*` | none; optionally `OLLAMA_BASE_URL` for a custom local/remote endpoint | Local no-auth fallback; default model is `ollama/qwen3.5:cloud` |
| OpenAI Responses | `codex/*` | `OPENAI_API_KEY` (or `OPENAI_BASE_URL` for a custom OpenAI-compatible endpoint) | Responses API — newer agentic-native shape; custom keyless endpoints are supported when the endpoint accepts unauthenticated requests |
