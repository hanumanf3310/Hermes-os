...
> พฤติกรรมของ `thclaws-safe`: เมื่อไม่มี `OPENAI_API_KEY` จะลองใช้ local OpenAI-compatible endpoint ที่ตอบได้ก่อน; ถ้าไม่พบจึง fallback ไป Ollama local (`ollama/qwen3.5:cloud`).
...
| OpenAI | `gpt-*`, `o1-*`, `o3*`, `o4-*` | `OPENAI_API_KEY` (หรือ `OPENAI_BASE_URL` หากชี้ไปยัง endpoint แบบ OpenAI-compatible ที่กำหนดเอง) | Chat Completions; prompt caching อัตโนมัติ; ถ้า endpoint ปลายทางไม่บังคับ auth ก็ใช้งานแบบไม่ใส่ key ได้ |
| Ollama | `ollama/*` | ไม่มี; จะมี `OLLAMA_BASE_URL` ให้ถ้าชี้ endpoint local/remote แบบกำหนดเอง | ตัว fallback แบบไม่ต้องใช้ auth; default model คือ `ollama/qwen3.5:cloud` |
| OpenAI Responses | `codex/*` | `OPENAI_API_KEY` (หรือ `OPENAI_BASE_URL` หากชี้ไปยัง endpoint แบบ OpenAI-compatible ที่กำหนดเอง) | Responses API — รูปแบบ agentic-native ที่ใหม่กว่า; ใช้แบบ keyless ได้เมื่อปลายทางรับคำขอแบบไม่ต้อง auth |
