...
Use this when Boss asks to study a large external Agent Skills repository, marketplace, external agent harness/workspace, or interactive agent workflow blueprint/course and explain how to apply it to Hermes OS.

This class includes downloaded local folders that teach Claude Code or another coding agent through `CLAUDE.md` / `AGENTS.md` instructions, lesson modules, slash commands, skills, MCPs, subagents, deployment, or similar guided build flows.
...
### 3C. Inspect interactive agent workflow blueprints/courses

Use this path when Boss provides a downloaded local folder or URL for an interactive Claude Code / agentic workflow blueprint rather than a conventional repo or skill catalog.

Evidence to inspect first:

- root instruction files: `CLAUDE.md`, `AGENTS.md`, `README.md`, `COURSE-PLAN.md`, `Course-Outcomes.md`, or equivalents
- lesson/module files, especially `lesson-modules/*/CLAUDE.md`
- slash-command definitions such as `.claude/commands/*.md`
- skill definitions such as `.claude/skills/*/SKILL.md`
- MCP setup instructions, deployment steps, and external service requirements
- missing files mentioned by docs, for example absent `_source-drafts/` despite being listed

Report the course as a system pattern, not just content:

```text
root instructor context
  → ordered lesson modules
  → STOP / USER / ACTION pacing blocks
  → user creates concrete artifacts
  → verification checkpoint per lesson
  → final deploy/public proof
```

For Claude Code Blueprint-style folders, useful extraction points are:

- the learner-facing promise and final artifact
- exact file tree discovered from disk
- lesson order and hard prerequisites
- what each lesson creates
- how `CLAUDE.md` is used as standing project context
- how slash commands compress repeated prompts
- how skills encode reusable job procedure
- how MCPs connect external systems
- how subagents are dispatched in parallel, ideally in one tool-use message
- how deployment is verified as public rather than local-only
- explicit non-goals such as no auth, no scheduling, no retries, no production hardening
- reusable teaching mechanics: `STOP:`, `USER:`, `ACTION:`, common stumbles, end-of-lesson evidence checks

Hermes OS adaptation lens:

```text
agent blueprint pattern
  → Hermes policy/context binding
  → slash command or skill protocol
  → external adapter/MCP
  → delegate_task/Fleet parallel work
  → evidence store or dashboard
  → checkpointed final proof
```

Recommended final report sections:

1. verified source path / URL and files found
2. one-paragraph purpose
3. artifacts the learner builds
4. lesson-by-lesson flow
5. instructional control pattern
6. dependencies and safety gaps
7. Hermes OS reuse opportunities
8. what is inferred vs directly verified

Do not claim the blueprint works end-to-end unless you actually run the lesson flow and verify the artifacts. Reading files supports a content/architecture summary only.
