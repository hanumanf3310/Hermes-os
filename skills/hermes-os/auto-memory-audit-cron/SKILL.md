...
#### Primary Path: `memory()` + `fact_store()` tools

```python
# Try native tools first
memory(action="add", target="memory", content="...")
fact_store(action="add", category="user_pref", content="...", tags="a,b,c")
```

#### Fallback Path: Direct JSONL append via `execute_code`

When `memory()` is **unavailable** (disabled in config) or `fact_store()` is **not in the toolset**:
...
facts = [{
    "id": f"fact_memaudit_{now.strftime('%Y%m%d_%H%M%S')}_suffix",
    "timestamp": now.isoformat(),
    "session_id": "session_id_here",
    "category": "user_pref|project|tool|general",
    "importance": 4,
    "tags": "tag1,tag2,tag3",
    "content": "[5W1H] WHO: X. WHAT: Y. WHEN: ... WHERE: ... WHY: ... HOW: ..."
}]

fs_path = os.path.expanduser("~/.hermes/fact_store.jsonl")
with open(fs_path, "a", encoding="utf-8") as f:
    for fact in facts:
        f.write(json.dumps(fact, ensure_ascii=False) + "\n")
```

**Always use fallback when you see:**
- `Memory is not available. It may be disabled in config or this environment.`
- `Tool 'fact_store' does not exist. Available tools: [...]`
...
### Case 5: `memory()` or `fact_store()` tools unavailable
```
Action: Fall back to direct execute_code JSONL append to ~/.hermes/fact_store.jsonl
Reason: Hermes configs vary per deployment; JSONL is the durable common denominator
Always probe tool availability first, never assume
```
...
### Issue 4: Distinguishing User vs Tool/Assistant Signals

**Problem:** Session files contain completion signals from multiple sources:
- Tool outputs: `{"success": true}`, `"status": "completed"`
- Assistant responses: "พร้อมแล้วค่ะ", "done"
- User messages: "เสร็จแล้ว", "เรียบร้อย"

Only **user-generated** completion signals should be saved as memories.

**Solution:** When parsing session files:
1. Check `role == "user"` for completion signals
2. Ignore tool outputs with `"success": true` unless user explicitly confirms
3. Ignore assistant responses saying "done" - look for Boss/user confirmation
4. Look for explicit user messages with completion vocabulary

**Example filtering:**
```python
for line in session_file:
    msg = json.loads(line)
    if msg.get('role') == 'user':  # Only process user messages
        content = msg.get('content', '')
        # Check for completion signals here
```
...
```json
{
  ...
  "content": "[5W1H] WHO: Hermes Agent. WHAT: Repository prepared for actual update. WHERE: /home/hanuman3310/.hermes/hermes-agent. WHEN: 2026-04-27T05:42:00+07:00. WHY: Boss requested full update preparation. HOW: Safety commit + rebase + clean working tree + test validation.",
  "5W1H": {
