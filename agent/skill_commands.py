...
def reload_skills() -> Dict[str, Any]:
    """Re-scan skill slash commands and return a before/after diff.

    This refreshes the in-process ``/skill-name`` command map without
    invalidating the skills prompt-cache snapshot. Skills are invoked at
    runtime via slash commands, ``skills_list``, and ``skill_view``; they do
    not need to be re-rendered into the system prompt to become usable.
    """
    ...
    return {
        "added": [{"name": name, "description": after[name]} for name in added_names],
        "removed": [{"name": name, "description": before[name]} for name in removed_names],
        "unchanged": unchanged,
        "total": len(after),
